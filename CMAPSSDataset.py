...
class CMAPSSDataset():
    def __init__(self, fd_number, batch_size, sequence_length, deleted_engine, feature_columns):
        ...
        norm_test_data = pd.DataFrame(self.std.transform(test_data[cols_normalize]), columns=cols_normalize,
    ...
    def get_label_slice(self, data):
        ...
        for i in range(1, self.engine_size + 1):
            ...
            for j in range(0, selected_label_data.shape[0] - self.sequence_length + 1):
                cur = selected_label_data[j + self.sequence_length - 1]
                cur = 125 if cur > 125 else cur
                label_list.append(cur/125)
...
def get_features_dim(FD='1'):
    if FD == '1':
        sequence_length = 30
        FD_feature_columns = ['s1', 's2', 's3', 's4', 's5', 's7', 's8', 's9', 's11', 's12', 's13', 's14', 's15', 's17',
                              's20', 's21']
    ...
    return sequence_length, FD_feature_columns, len(FD_feature_columns)


def get_data(FD='1', sequence_length=30, feature_columns=[], batch_size=1024, label='train'):
    datasets = CMAPSSDataset(fd_number=FD, batch_size=batch_size, sequence_length=sequence_length,
                             deleted_engine=[1000], feature_columns=feature_columns)
    if label == 'train':
        train_data = datasets.get_train_data()
        train_feature_slice = datasets.get_feature_slice(train_data)
        train_label_slice = datasets.get_label_slice(train_data)
        return train_feature_slice, train_label_slice
    else:
        test_data = datasets.get_test_data()
        test_feature_slice = datasets.get_feature_slice(test_data)
        test_label_slice = datasets.get_label_slice(test_data)
        return test_feature_slice, test_label_slice
...
if __name__ == "__main__":
    ...
    if FD == '1':
        sequence_length = 31
        FD_feature_columns = ['s2', 's3', 's4', 's6', 's7', 's8', 's9', 's11', 's12', 's13', 's14', 's15', 's17', 's20',
                              's21']
    ...
    datasets = CMAPSSDataset(fd_number=FD, batch_size=batch_size, sequence_length=sequence_length,
                             deleted_engine=[1000],
                             feature_columns=FD_feature_columns)  # deleted_engine=[5,17,31,41,46,55,69,73,82,95]
